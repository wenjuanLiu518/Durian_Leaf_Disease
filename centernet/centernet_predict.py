...
from centernet_tools import model_by, predict_by
...
if __name__ == '__main__':
    run_dir = r"centernet\predict"
    if not osp.exists(run_dir): os.makedirs(run_dir)
    logger_file(osp.join(run_dir, "centernet_predict_log.log"))
    _model_ = model_by(num_classes=1, model_path=makd_abs_path(r"centernet\train\weights\best.pth"))
    logger(_model_)
