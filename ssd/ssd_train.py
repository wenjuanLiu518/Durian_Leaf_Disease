...
import shutil
from ssd_tools import model_by, train_by
...
if __name__ == '__main__':
    ...
    logger_file(osp.join(run_dir, "ssd_train_log.log"))
    # 配置last_wieght_path就会继续训练
    last_weight_path = osp.join(run_dir, "weights", "last.pth")
    _model_ = model_by(model_path=last_weight_path, required_size=IMG_SIZE, num_classes=len(VOC_CLASSES))
    logger(_model_)
