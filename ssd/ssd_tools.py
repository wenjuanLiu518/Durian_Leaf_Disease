...
from voc_dataset import VOC_CLASSES, VOC2007Dataset, collate_fn, IMG_SIZE
from utils import logger

def makd_abs_path(fn):
    return osp.abspath(osp.join(osp.dirname(__file__), fn))

def model_by(required_size=IMG_SIZE, num_classes=len(VOC_CLASSES), model_path:str=None, backbone_weights=None):
    """
    1、特征图fpn
    2、注意力机制
    """
    # resnet18
    if backbone_weights is None: backbone_weights = ResNet18_Weights.DEFAULT
    model_backbone = resnet18(weights=backbone_weights)
...
def train_by(model, data_path:str, num_epochs=100, last_epoch=-1, **kwargs):
    ...
    optimizer = optim.SGD(params, lr=0.0010, momentum=0.9, weight_decay=0.0005, nesterov=True)
    # epoch_lr_s = CosineAnnealingLR(optimizer=optimizer, T_max=num_epochs, eta_min=0.00005)
    epoch_lr_s = MultiStepLR(optimizer=optimizer, milestones=[num_epochs/4, num_epochs/2, 3*num_epochs/4], gamma=0.5)
    weight_dir = makd_abs_path(r"ssd\train\weights") if "out_dir" not in kwargs else kwargs["out_dir"]
    if not osp.exists(weight_dir): os.makedirs(weight_dir)
    # 数据集配置
    dataset_train = VOC2007Dataset(root=data_path, image_set='train', transforms=F_train_transforms, run_dir=makd_abs_path(r"ssd\train"))
    dataloader_train = DataLoader(dataset_train, batch_size=32, num_workers=4, shuffle=True, collate_fn=collate_fn)
...
def val_a_epoch(model, dataloader, epoch, num_epochs):
    ...
    for images, targets in tqdm.tqdm(dataloader, file=sys.stdout):
        ...
        for i in range(len(images)):
            trues_dict = dict()
            preds_dict = dict()
            trues_dict['boxes'] = targets[i]['boxes'].detach().cpu()
            trues_dict['labels'] = targets[i]['labels'].detach().cpu()
            preds_dict['boxes'] = outputs[i]['boxes'].detach().cpu()
            preds_dict['scores'] = outputs[i]['scores'].detach().cpu()
            preds_dict['labels'] = outputs[i]['labels'].detach().cpu()
            preds.append(preds_dict)
            trues.append(trues_dict)
    # 清空缓存
    torch.cuda.empty_cache()
    # 计算指标
...
def predict_by(model, source_dir, trues_dict=None, **kwargs):
    ...
    predict_dir = makd_abs_path(r"ssd\predict") if "out_dir" not in kwargs else kwargs["out_dir"]
