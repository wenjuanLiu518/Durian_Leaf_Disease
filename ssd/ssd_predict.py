...
from ssd_tools import model_by, predict_by
from voc_dataset import VOC_CLASSES, IMG_SIZE
...
if __name__ == '__main__':
    run_dir = r"ssd\predict"
    if not osp.exists(run_dir): os.makedirs(run_dir)
    logger_file(osp.join(run_dir, "ssd_predict_log.log"))
    _model_ = model_by(required_size=IMG_SIZE, num_classes=len(VOC_CLASSES), model_path=makd_abs_path(r"ssd\train\weights\best.pth"))
    logger(_model_)
